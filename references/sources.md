...
第一章只使用无品牌的 GEMM/Attention 到模型级 FLOPs、NVIDIA GPU 理论时间下界、显存生命周期、数据移动和通信直觉；具体训推工作单元、指标与案例分别属于 02、03。H100 峰值、带宽和第一章通用演算已登记为 `FACT-005`；02 仍需登记 MFU/HFU 与训练案例口径，03 仍需登记推理指标与案例口径，不得使用一般化 MFU 区间或实时预算。Qwen3-32B 规格已核验；H3 与 Qwen3-32B 的规模对比已登记为 `FACT-004`，在统一口径和来源定位补齐前不得填入精确数值或表述为严格相等。
